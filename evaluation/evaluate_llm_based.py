# evaluate_llm_based.py - LLM-Based Chatbot Evaluation Script
# ทดสอบ LLM-Based Chatbot โดยเฉพาะด้วยคำถามที่ไม่มีคีย์เวิร์ด (test_forRetriver_no_keywords.json)
# เพื่อพิสูจน์ว่า LLM สามารถเข้าใจและจำแนก Intent ได้แม้ไม่มีคีย์เวิร์ดชัดเจน
...
# Import LLM-Based Chatbot
print("📦 [INFO] Loading LLM-Based Chatbot...")
...
try:
    ...
    print(f"❌ [ERROR] LLM-Based Chatbot not available: {e}")
    sys.exit(1)
...
def run_evaluation(test_questions: List[Dict], max_contexts: int = None) -> Dict[str, Any]:
    """
    รัน evaluation สำหรับ LLM-Based chatbot
    ...
    if not LLM_BASED_AVAILABLE:
        print("❌ Cannot run evaluation without LLM-Based Chatbot")
        return None
    
    questions = test_questions
    print(f"\n{'='*80}")
    print(f"📝 กำลังทดสอบ LLM-Based Chatbot ด้วย {len(questions)} คำถาม")
    ...
    for i, test_case in enumerate(questions, 1):
        ...
        try:
            answer = llm_chatbot.answer(question)
        ...
        contexts, total_retrieved = get_contexts_from_chatbot(llm_chatbot, question, max_contexts)
    ...
    print(f"\n{'='*80}")
    print(f"📊 Running RAGAS evaluation for LLM-Based...")
    print(f"{'='*80}")
...
def print_results(results: Dict[str, Any]):
    ...
    print("\n" + "="*80)
    print("📊 LLM-Based Chatbot Results:")
    print("="*80)
...
def save_results(results: Dict[str, Any], max_contexts: int = None, filename: str = None):
    ...
    if filename is None:
        ...
        filename = f"llm_based_evaluation{ctx_suffix}_{timestamp}.json"
...
def main():
    ...
    print("\n" + "="*80)
    print("🤖 LLM-Based Chatbot Evaluation")
    print("="*80)
    print("Testing LLM's ability to handle questions WITHOUT clear keywords")
    print("Dataset: test_forRetriver_no_keywords.json (21% modified questions)")
    print()
    
    if not RAGAS_AVAILABLE or not LLM_BASED_AVAILABLE:
    ...
    print(f"   🎯 Focus: LLM-Based intent classification (no keyword dependency)")
    print()
    ...
    if results:
        ...
        print(f"   - Expected: LLM should handle questions well even without keywords!")
    else:
        print("\n❌ Evaluation failed")
